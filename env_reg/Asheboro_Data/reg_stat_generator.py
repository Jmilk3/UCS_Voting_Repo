...
def showVotingStats(dataframe: pd.DataFrame, precinct: str) -> str:
    ...
    if precinct not in dataframe.precinct_name.values:
        ...
        # Remove overvote, undervote, and misc writein
        data = data[dataframe["candidate_name"].isin(["OVER VOTE", "UNDER VOTE", "Write-In (Miscellaneous)"]) == False]
...
# Define some values when importing this file
edu_data = pd.read_csv("board_of_edu_data_clean.csv")
council_data = pd.read_csv("city_council_data_clean.csv")
